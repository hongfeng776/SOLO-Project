...
import type { IStockProductBatchImportResult } from '@/types/api'
...
const importResult = ref<IStockProductBatchImportResult | null>(null)
...
const progressStatus = computed(() => {
  const pct = totalData.value > 0 ? Math.floor((currentProgress.value / totalData.value) * 100) : 0
...
async function handleStartImport() {
  ...
  const batchSize = 50
  const result: IStockProductBatchImportResult = {
  ...
  for (let i = 0; i < fileData.value.length; i += batchSize) {
    ...
    try {
      const res = await stockProductApi.batchImport(batch)
      if (res.code === 0) {
...
function exportErrorReport() {
  ...
  const link = document.createElement('a')
  link.href = url
  link.download = `股票产品档案导入错误报表_${Date.now()}.json`
  link.click()
  URL.revokeObjectURL(url)
