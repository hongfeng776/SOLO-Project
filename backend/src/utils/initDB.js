...
const { User, Role, CapacityType, Driver, Passenger, Vehicle, Order, FinanceStatement, FinanceSettlement, Coupon, RiskRule, MarketingCampaign, MarketingAuditLog, MarketingAudienceLog, MarketingRedemptionRecord, Notification, Ticket } = require('../models')
...
const initDB = async () => {
  try {
    ...
    await MarketingCampaign.bulkCreate([
      {
        ...
        orderCount: 3180,
        impressionCount: 256800,
        clickCount: 68500,
        conversionCount: 2980,
        conversionAmount: 96820.00,
        roiValue: 2.97,
        ctrValue: 0.2667,
        conversionRate: 0.9150,
        redemptionRate: 0.9767,
        efficiencyLevel: 5,
        efficiencyScore: 93.5,
        optimizeSuggestions: JSON.stringify([
          { type: 'template', level: 'success', priority: 1, title: '优质拉新模板', detail: '核销率高达97.67%，ROI接近3倍，建议复制到其他一线城市' },
          { type: 'strategy', level: 'info', priority: 3, title: '覆盖范围扩展', detail: '当前定向新用户过于精准，可考虑加入"注册30-60天"标签进一步扩张' }
        ]),
        dataAuthenticity: 2,
        authenticityScore: 94,
        fraudWarningCount: 3,
        isTemplate: true,
        templateTags: ['高ROI', '高核销率', '拉新效果好', '数据纯净'],
        funnelData: JSON.stringify({
          stages: [
            { key: 'impression', label: '活动曝光', value: 256800, color: '#66b1ff' },
            { key: 'click', label: '点击进入', value: 68500, color: '#67c23a' },
            { key: 'participate', label: '参与活动', value: 3256, color: '#e6a23c' },
            { key: 'receive', label: '领取权益', value: 3256, color: '#8e44ad' },
            { key: 'use', label: '核销使用', value: 3180, color: '#f56c6c' },
            { key: 'conversion', label: '订单转化', value: 2980, color: '#ff0050' }
          ]
        }),
        lastEvaluatedAt: new Date('2025-01-10 09:15:30'),
        creatorId: 1,
        creatorName: '超级管理员',
      ...
      },
      {
        ...
        orderCount: 42300,
        impressionCount: 586000,
        clickCount: 152000,
        conversionCount: 12800,
        conversionAmount: 285600.00,
        roiValue: 3.19,
        ctrValue: 0.2594,
        conversionRate: 0.8101,
        redemptionRate: 0.8354,
        efficiencyLevel: 4,
        efficiencyScore: 82.3,
        optimizeSuggestions: JSON.stringify([
          { type: 'conversion', level: 'warning', priority: 3, title: '转化链路优化', detail: '核销率83.5%良好，但转化率从领取到转化流失近17%，建议增加权益到期提醒推送' },
          { type: 'budget', level: 'info', priority: 4, title: '预算利用率', detail: '当前预算使用44.8%，建议增加曝光量进一步提升活动覆盖' }
        ]),
        dataAuthenticity: 2,
        authenticityScore: 88,
        fraudWarningCount: 18,
        isTemplate: true,
        templateTags: ['高ROI', '节日营销模板'],
        funnelData: JSON.stringify({
          stages: [
            { key: 'impression', label: '活动曝光', value: 586000, color: '#66b1ff' },
            { key: 'click', label: '点击进入', value: 152000, color: '#67c23a' },
            { key: 'participate', label: '参与活动', value: 15800, color: '#e6a23c' },
            { key: 'receive', label: '领取权益', value: 15800, color: '#8e44ad' },
            { key: 'use', label: '核销使用', value: 13200, color: '#f56c6c' },
            { key: 'conversion', label: '订单转化', value: 12800, color: '#ff0050' }
          ]
        }),
        lastEvaluatedAt: new Date('2025-01-12 08:30:00'),
        creatorId: 2,
        creatorName: '运营管理员',
      ...
      },
      {
        ...
        orderCount: 98500,
        impressionCount: 1820000,
        clickCount: 326800,
        conversionCount: 45200,
        conversionAmount: 685400.00,
        roiValue: 2.68,
        ctrValue: 0.1796,
        conversionRate: 0.7958,
        redemptionRate: 0.8486,
        efficiencyLevel: 3,
        efficiencyScore: 71.6,
        optimizeSuggestions: JSON.stringify([
          { type: 'ctr', level: 'warning', priority: 3, title: '点击率优化', detail: 'CTR仅17.96%低于行业20%基准，建议优化banner素材或个性化推荐算法' },
          { type: 'conversion', level: 'info', priority: 4, title: '复购引导', detail: '核销后7日复购率偏低，建议核销后推送额外5元优惠' }
        ]),
        dataAuthenticity: 3,
        authenticityScore: 68,
        fraudWarningCount: 96,
        isTemplate: false,
        templateTags: [],
        funnelData: JSON.stringify({
          stages: [
            { key: 'impression', label: '活动曝光', value: 1820000, color: '#66b1ff' },
            { key: 'click', label: '点击进入', value: 326800, color: '#67c23a' },
            { key: 'participate', label: '参与活动', value: 42600, color: '#e6a23c' },
            { key: 'receive', label: '领取权益', value: 56800, color: '#8e44ad' },
            { key: 'use', label: '核销使用', value: 48200, color: '#f56c6c' },
            { key: 'conversion', label: '订单转化', value: 45200, color: '#ff0050' }
          ]
        }),
        lastEvaluatedAt: new Date('2025-01-15 10:20:00'),
        creatorId: 2,
        creatorName: '运营管理员',
      ...
      },
      {
        ...
        orderCount: 3800,
        impressionCount: 42800,
        clickCount: 6800,
        conversionCount: 2200,
        conversionAmount: 68200.00,
        roiValue: 0.99,
        ctrValue: 0.1589,
        conversionRate: 0.5789,
        redemptionRate: 0.7308,
        efficiencyLevel: 1,
        efficiencyScore: 38.7,
        optimizeSuggestions: JSON.stringify([
          { type: 'roi', level: 'danger', priority: 1, title: 'ROI严重不达标', detail: '当前ROI仅0.99，活动处于亏损状态，建议立即评估补贴额度是否过高' },
          { type: 'redemption', level: 'warning', priority: 2, title: '核销率偏低', detail: '召回礼包补贴高达20元但核销率仅73%，权益可能对用户吸引力不足' },
          { type: 'conversion', level: 'warning', priority: 2, title: '转化流失严重', detail: '领取到转化流失42%，建议增加首单完成后的二次奖励阶梯设计' },
          { type: 'risk', level: 'info', priority: 4, title: '风控排查', detail: '建议核验召回人群标签中是否混入了低价值羊毛党' }
        ]),
        dataAuthenticity: 4,
        authenticityScore: 42,
        fraudWarningCount: 86,
        isTemplate: false,
        templateTags: [],
        funnelData: JSON.stringify({
          stages: [
            { key: 'impression', label: '活动曝光', value: 42800, color: '#66b1ff' },
            { key: 'click', label: '点击进入', value: 6800, color: '#67c23a' },
            { key: 'participate', label: '参与活动', value: 5200, color: '#e6a23c' },
            { key: 'receive', label: '领取权益', value: 5200, color: '#8e44ad' },
            { key: 'use', label: '核销使用', value: 3800, color: '#f56c6c' },
            { key: 'conversion', label: '订单转化', value: 2200, color: '#ff0050' }
          ]
        }),
        lastEvaluatedAt: new Date('2025-01-18 14:50:00'),
        creatorId: 1,
        creatorName: '超级管理员',
        onlineTime: null,
    ...
    await MarketingRedemptionRecord.bulkCreate([
      {
        id: 1,
        campaignId: 1,
        campaignName: '新用户首单优惠',
        userId: 101,
        userPhone: '13812345001',
        userLevel: 1,
        orderId: 2001,
        orderNo: 'ORD20250101001',
        redemptionCode: 'RDM-NEW-001',
        status: 5,
        auditLevel: 1,
        violationType: 'none',
        complianceChecks: JSON.stringify([
          { type: 'eligibility', name: '用户参与资格', passed: true, message: '用户参与资格校验通过', detail: '' },
          { type: 'expiry', name: '权益有效期', passed: true, message: '权益有效期校验通过', detail: '' },
          { type: 'scene', name: '使用场景匹配', passed: true, message: '使用场景校验通过', detail: '' },
          { type: 'duplicate', name: '重复核销检测', passed: true, message: '重复核销检测通过', detail: '' },
          { type: 'amount', name: '金额合规校验', passed: true, message: '金额合规校验通过', detail: '' },
          { type: 'frequency', name: '频次限额校验', passed: true, message: '频次限额校验通过', detail: '' }
        ]),
        complianceScore: 100,
        scene: 1,
        vehicleType: '快车',
        ...
        orderAmount: 35.00,
        benefitStartTime: new Date('2025-01-01'),
        benefitEndTime: new Date('2025-12-31'),
        participateTime: new Date('2025-01-02 09:30:00'),
        receiveTime: new Date('2025-01-02 09:31:00'),
        redemptionTime: new Date('2025-01-02 10:15:00'),
        ipAddress: '116.28.123.45',
        deviceId: 'DEV-A1B2C3D4E5',
        isRevocable: 0
      },
      {
        id: 2,
        ...
        campaignName: '元旦出行红包',
        userId: 202,
        userPhone: '13912345002',
        userLevel: 3,
        orderId: 2002,
        orderNo: 'ORD20250101002',
        redemptionCode: 'RDM-HOL-002',
        status: 2,
        auditLevel: 2,
        violationType: 'cross_scenario',
        complianceChecks: JSON.stringify([
          { type: 'eligibility', name: '用户参与资格', passed: true, message: '用户参与资格校验通过', detail: '' },
          { type: 'expiry', name: '权益有效期', passed: true, message: '权益有效期校验通过', detail: '' },
          { type: 'scene', name: '使用场景匹配', passed: false, message: '核销城市不在活动适用范围内', detail: '核销城市：天津，适用：北京、上海、广州、深圳、杭州、成都' },
          { type: 'duplicate', name: '重复核销检测', passed: true, message: '重复核销检测通过', detail: '' },
          { type: 'amount', name: '金额合规校验', passed: true, message: '金额合规校验通过', detail: '' },
          { type: 'frequency', name: '频次限额校验', passed: true, message: '频次限额校验通过', detail: '' }
        ]),
        ...
        vehicleType: '快车',
        city: '天津',
        redemptionAmount: 5.00,
        orderAmount: 22.00,
        benefitStartTime: new Date('2024-12-25'),
        benefitEndTime: new Date('2025-01-15'),
        participateTime: new Date('2025-01-01 08:00:00'),
        receiveTime: new Date('2025-01-01 08:01:00'),
        redemptionTime: new Date('2025-01-01 08:45:00'),
        ipAddress: '117.15.234.56',
        deviceId: 'DEV-F6G7H8I9J0',
        isRevocable: 1
      },
      {
        id: 3,
        campaignId: 3,
        campaignName: '日常出行普惠补贴',
        userId: 303,
        userPhone: '13712345003',
        userLevel: 2,
        orderId: 2003,
        orderNo: 'ORD20250102003',
        redemptionCode: 'RDM-DLY-003',
        status: 1,
        auditLevel: 1,
        violationType: 'none',
        complianceChecks: JSON.stringify([
        ...
        vehicleType: null,
        city: '上海',
        redemptionAmount: 3.00,
        orderAmount: 18.50,
        benefitStartTime: new Date('2025-01-01'),
        benefitEndTime: new Date('2025-03-31'),
        participateTime: new Date('2025-01-05 17:30:00'),
        receiveTime: new Date('2025-01-05 17:31:00'),
        redemptionTime: new Date('2025-01-05 18:20:00'),
        ipAddress: '114.88.156.78',
        deviceId: 'DEV-K1L2M3N4O5',
        isRevocable: 1
      },
      {
        id: 4,
        ...
        userLevel: 2,
        orderId: 2004,
        orderNo: 'ORD20250102004',
        redemptionCode: 'RDM-DLY-004',
        status: 3,
        auditLevel: 2,
        ...
        complianceChecks: JSON.stringify([
          ...
          { type: 'expiry', name: '权益有效期', passed: true, message: '权益有效期校验通过', detail: '' },
          { type: 'scene', name: '使用场景匹配', passed: true, message: '使用场景校验通过', detail: '' },
          { type: 'duplicate', name: '重复核销检测', passed: false, message: '检测到重复核销：同一用户对同一订单已核销', detail: '已存在1条核销记录' },
          { type: 'amount', name: '金额合规校验', passed: true, message: '金额合规校验通过', detail: '' },
          { type: 'frequency', name: '频次限额校验', passed: true, message: '频次限额校验通过', detail: '' }
        ]),
        complianceScore: 85,
        ...
        benefitStartTime: new Date('2025-01-01'),
        benefitEndTime: new Date('2025-03-31'),
        participateTime: new Date('2025-01-06 08:30:00'),
        receiveTime: new Date('2025-01-06 08:31:00'),
        redemptionTime: new Date('2025-01-06 09:10:00'),
        auditorId: 1,
        auditorName: '超级管理员',
        auditAt: new Date('2025-01-06 10:00:00'),
        rejectReason: '同一用户重复核销同一活动权益',
        ipAddress: '114.88.156.78',
        deviceId: 'DEV-K1L2M3N4O5',
        isRevocable: 0
      },
      {
        id: 5,
        campaignId: 4,
        campaignName: '流失用户召回礼包',
        userId: 404,
        userPhone: '13612345005',
        userLevel: 1,
        orderId: 2005,
        orderNo: 'ORD20250102005',
        redemptionCode: 'RDM-RCL-005',
        status: 2,
        auditLevel: 2,
        violationType: 'fake',
        complianceChecks: JSON.stringify([
          ...
          { type: 'expiry', name: '权益有效期', passed: true, message: '权益有效期校验通过', detail: '' },
          { type: 'scene', name: '使用场景匹配', passed: true, message: '使用场景校验通过', detail: '' },
          { type: 'duplicate', name: '重复核销检测', passed: false, message: '同一设备存在多个用户核销，疑似虚假核销', detail: '设备4D4E5F已关联4个不同用户' },
          { type: 'amount', name: '金额合规校验', passed: true, message: '金额合规校验通过', detail: '' },
          { type: 'frequency', name: '频次限额校验', passed: true, message: '频次限额校验通过', detail: '' }
        ]),
        complianceScore: 75,
        ...
        vehicleType: null,
        city: '广州',
        redemptionAmount: 20.00,
        orderAmount: 42.00,
        benefitStartTime: new Date('2025-01-15'),
        benefitEndTime: new Date('2025-02-28'),
        participateTime: new Date('2025-01-18 14:00:00'),
        receiveTime: new Date('2025-01-18 14:01:00'),
        redemptionTime: new Date('2025-01-18 14:30:00'),
        ipAddress: '183.6.112.33',
        deviceId: 'DEV-P6Q7R8S9T0',
        isRevocable: 1
      },
      {
        id: 6,
        ...
        campaignName: '流失用户召回礼包',
        userId: 405,
        userPhone: '15512345006',
        userLevel: 1,
        orderId: null,
        orderNo: null,
        redemptionCode: 'RDM-RCL-006',
        status: 0,
        auditLevel: 0,
        violationType: 'none',
        ...
        benefitStartTime: new Date('2025-01-15'),
        benefitEndTime: new Date('2025-02-28'),
        participateTime: new Date('2025-01-20 10:00:00'),
        receiveTime: new Date('2025-01-20 10:01:00'),
        redemptionTime: null,
        ipAddress: null,
        deviceId: null,
        isRevocable: 1
