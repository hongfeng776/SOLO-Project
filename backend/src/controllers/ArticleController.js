...
class ArticleController {
  getList = [
    validate(paginationSchema.keys({
      ...
      domainCategory: Joi.string().trim().allow(null, ''),
      publishChannel: Joi.number().integer().valid(1, 2, 3, 4).allow(null, ''),
      publishPermission: Joi.number().integer().valid(0, 1, 2, 3).allow(null, ''),
  ...
  create = [
    validate(Joi.object({
      ...
      articleType: Joi.number().integer().valid(0, 1, 2, 3, 4).allow(null, ''),
      domainCategory: Joi.string().valid('tech', 'entertainment', 'sports', 'finance', 'lifestyle', 'education').allow(null, ''),
      publishChannel: Joi.number().integer().valid(1, 2, 3, 4).allow(null, ''),
      publishPermission: Joi.number().integer().valid(0, 1, 2, 3).allow(null, ''),
      ...
      articleQuality: Joi.number().integer().valid(0, 1, 2, 3).allow(null, ''),
      layoutTemplate: Joi.string().valid('default', 'full-width', 'magazine', 'elegant', 'tech-style').allow(null, ''),
      summary: Joi.string().max(500).allow(null, ''),
      coverImages: Joi.array().items(Joi.string()).allow(null, ''),
      contentImages: Joi.array().items(Joi.string()).allow(null, ''),
      creatorUid: Joi.string().allow(null, ''),
      creatorLevel: Joi.number().integer().valid(0, 1, 2, 3, 4).allow(null, ''),
      resourcePosition: Joi.string().allow(null, ''),
      imageResolution: Joi.string().allow(null, ''),
    })),
    async (req, res, next) => {
      try {
